/**
 * Plan 17-04 — Void event dialog (EVA-04).
 *
 * Justification-only clone of the resolve dialog using the SERVER two-step:
 * "Preview impact" calls `voidEvent({confirm:false})` (every child settles NO —
 * explicitly NOT a stake refund) and shows the projected impact; the destructive
 * "Confirm void" calls `voidEvent({confirm:true})`. Editing the justification
 * clears the preview.
 */
"use client";
...
import { voidEvent } from "@/lib/admin-events-api";
...
export function VoidEventDialog({
  ...
  function mapError(err: unknown) {
    if (isSessionExpiredError(err)) {
      ...
      toast.error("Mirrored events are read-only and can't be voided here.");
    } else {
      toast.error("Couldn't void the event. Please try again.");
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !submitting && onOpenChange(o)}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Void event</DialogTitle>
          <DialogDescription>
            Voiding settles every outcome on NO (YES bettors lose, NO bettors
            win). This is not a stake refund and cannot be undone without a
            reversal.
          </DialogDescription>
        </DialogHeader>
