/**
 * shadcn/ui DropdownMenu primitive (Radix `@radix-ui/react-dropdown-menu`).
 * Mirrors https://ui.shadcn.com/docs/components/dropdown-menu (verbatim except
 * the `@/lib/utils` import alias). Used for the CSV export menu. Only the
 * sub-components actually consumed in Phase 8 are exported (Root, Trigger,
 * Content, Item, Label, Separator) plus the standard composite set.
 */
"use client";
...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.SubContent
    ref={ref}
    className={cn(
      "z-50 min-w-[8rem] overflow-hidden rounded-md border border-zinc-200 bg-white p-1 text-zinc-950 shadow-lg data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-50",
      className,
    )}
    {...props}
  />
